//! Screenshot capture module using Wayland wlr-screencopy protocol
...
impl Dispatch<zwlr_screencopy_frame_v1::ZwlrScreencopyFrameV1, ()> for ScreenshotState {
    fn event(
        state: &mut Self,
...
delegate_noop!(ScreenshotState: ignore wl_shm::WlShm);
delegate_noop!(ScreenshotState: ignore wl_shm_pool::WlShmPool);
delegate_noop!(ScreenshotState: ignore wl_buffer::WlBuffer);
delegate_noop!(ScreenshotState: ignore zwlr_screencopy_manager_v1::ZwlrScreencopyManagerV1);
...
#[pyfunction]
pub fn capture<'py>(py: Python<'py>, output_name: &str) -> PyResult<Bound<'py, PyArray3<u8>>> {
...
#[pyfunction]
pub fn capture_raw(output_name: &str) -> PyResult<(Vec<u8>, u32, u32)> {
    capture_screenshot_impl(output_name)
}

/// List available outputs
#[pyfunction]
pub fn list_outputs() -> PyResult<Vec<String>> {
    let conn = Connection::connect_to_env()
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Failed to connect to Wayland: {}", e)))?;
